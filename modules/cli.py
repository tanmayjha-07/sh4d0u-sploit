...
def _run_dependency_installer(config: AppConfig, component_keys: list[str]) -> None:
    ...
    console.print(
        "[dim]If tools are still not detected, open a new terminal and run sh4d0u-sploit again "
...
def update_me(config: AppConfig) -> None:
    if not confirm(
        "Run [cyan]git fetch[/cyan] and [cyan]git rebase[/cyan] to update sh4d0u-sploit? "
        "Uncommitted local changes may conflict or be lost."
    ):
        return
    console.print("[yellow]Updating sh4d0u-sploit...[/yellow]")
    ...
    console.print("[cyan]Please restart sh4d0u-sploit.[/cyan]")
    config.run = False
